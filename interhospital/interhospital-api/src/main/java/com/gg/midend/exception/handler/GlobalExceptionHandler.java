package com.gg.midend.exception.handler;
...
import static com.gg.midend.exception.enums.GlobalErrorCodeConstants.*;

/**
 * 全局异常处理器，将 Exception
 * 翻译成 CommonResult + 对应的异常编号
 *
 * @author fun-mean
 */
...
public class GlobalExceptionHandler {
    ...
    public ApiResult<?> missingServletRequestParameterExceptionHandler(MissingServletRequestParameterException ex) {
        ...
        return ApiResult.error(BAD_REQUEST.getRspCode(), String.format("请求参数缺失:%s", ex.getParameterName()));
    }
    ...
    public ApiResult<?> methodArgumentTypeMismatchExceptionHandler(MethodArgumentTypeMismatchException ex) {
        log.warn("[missingServletRequestParameterExceptionHandler]", ex);
        return ApiResult.error(BAD_REQUEST.getRspCode(), String.format("请求参数类型错误:%s", ex.getMessage()));
    }
    ...
    public ApiResult<?> methodArgumentNotValidExceptionExceptionHandler(MethodArgumentNotValidException ex) {
        ...
        return ApiResult.error(BAD_REQUEST.getRspCode(), errorMsg);
    }
    ...
    public ApiResult<?> bindExceptionHandler(BindException ex) {
        ...
        assert fieldError != null; // 断言，避免告警
        return ApiResult.error(BAD_REQUEST.getRspCode(), String.format("请求参数不正确:%s", fieldError.getDefaultMessage()));
    }
    ...
    public ApiResult<?> constraintViolationExceptionHandler(ConstraintViolationException ex) {
        ...
        return ApiResult.error(BAD_REQUEST.getRspCode(), String.format("请求参数不正确:%s", constraintViolation.getMessage()));
    }

    /**
     * 处理 Dubbed Consumer 本地参数校验时，抛出的 ValidationException 异常
     */
    @ExceptionHandler(value = ValidationException.class)
    public ApiResult<?> validationException(ValidationException ex) {
        log.warn("[constraintViolationExceptionHandler]", ex);
        // 无法拼接明细的错误信息，因为 Dubbed Consumer 抛出 ValidationException 异常时，是直接的字符串信息，且人类不可读
    ...
    public ApiResult<?> noHandlerFoundExceptionHandler(HttpServletRequest req, NoHandlerFoundException ex) {
        ...
        return ApiResult.error(NOT_FOUND.getRspCode(), String.format("请求地址不存在:%s", ex.getRequestURL()));
    }
    ...
    public ApiResult<?> httpRequestMethodNotSupportedExceptionHandler(HttpRequestMethodNotSupportedException ex) {
        ...
        return ApiResult.error(METHOD_NOT_ALLOWED.getRspCode(), String.format("请求方法不正确:%s", ex.getMessage()));
    }
    ...
    public ApiResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        return ApiResult.error(INTERNAL_SERVER_ERROR.getRspCode(), INTERNAL_SERVER_ERROR.getRspMsg());
    }
