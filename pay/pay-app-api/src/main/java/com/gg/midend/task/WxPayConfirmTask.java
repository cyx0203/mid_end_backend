...
/**
 * 微信未确认状态支付、退费订单,自动确认,只 处理纯自费
 * 
 * @author 87392
 *
 */
@Component
public class WxPayConfirmTask {
	...
	/** 是否开启检测 */
	@Value("${SysConstants.WxPayAutoCheck:0}")
	private String WxPayAutoCheck;
	
	/**
	 * 5分钟确认一次微信未确认状态的支付、退费订单
	 * 
	 * @throws InterruptedException
	 */
	@Scheduled(cron = "0 0/5 * * * ?") // 每n分钟执行一次
	//@Scheduled(cron = "0/5 * * * * ? ") // 每n秒执行一次
	@SchedulerLock(name = "wxTask", lockAtMostFor = 300000, lockAtLeastFor = 300000)
	public void wxPayOrderStatusConfirmTask() throws InterruptedException {
		if ("0".equals(WxPayAutoCheck))
			return;
		GlobalConfig.log_api.info("wxPayOrderStatusConfirmTask 执行时间：{}", DateUtils.getDateTime("yyyy-MM-dd HH:mm:ss"));
		Map<String, Object> tradeParam = new HashMap<String, Object>();
		tradeParam.put("createDate", DateUtils.getDateTime("yyyyMMdd"));
		tradeParam.put("thirdId", "02");
	...
	/**
	 * 从所有未确认支付状态的微信交易记录中过滤掉医保拼付的记录 剔除与医保记录相同orderTrace的记录即可
	 * 此方法只能完全过滤支付订单，因为支付订单创建都是当日 而退费订单有隔日情况，只能过滤掉当日退费的拼付记录
	 * 
	 * @param recordList
	 *            查出pay_order_pay中当日所有status为0的微信记录
	 * @param recordYbList
	 *            当日医保记录
	 * @return
	 */
