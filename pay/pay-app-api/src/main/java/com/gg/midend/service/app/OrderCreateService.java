...
public class OrderCreateService {
	...
	public Map<String, Object> createOrder(Map<String, Object> requestMap) throws ApiException {
		...
		try {
			...
			String orderId = generateOrderTrace();
			...
			tradeParam.put("orderId", orderId);// 第三方支付服务方法都使用orderId,拼付时需要新生成一个
			...
			if (tradeParam.containsKey("spellFlag")) {// 拼付标志获取
				...
				String secOrderId = generateOrderTrace();
	...
	private boolean verifyPayType(Map<String, Object> tradeParam) throws Exception {
		...
		if (!payType.substring(0, 2).equals(thirdId))
			return false;
		if ("0102".equals(payType) || "0103".equals(payType) || "0104".equals(payType) || "0202".equals(payType) || "0203".equals(payType) || "0204".equals(payType) 
				|| "0105".equals(payType) || "0205".equals(payType) || "0401".equals(payType) || "06".equals(thirdId)) {
			return true;
		}
		return false;
	}
	...
	private String channelSecIdCheck(Map<String, Object> tradeParam) throws Exception {
		...
		if (merchantId == null || chanelType == null || secPayType == null
				|| GlobalParam.merchantWithChannelList == null)// 防止空指针
			return thisRealMerchantId;
		for (Object o : GlobalParam.merchantWithChannelList) {
			Map<Object, Object> oo = (Map) o;
			String thisMerchantId = Convert.convert(String.class, oo.get("inst_id"));
	...
	private boolean generatePayOrder(Map<String, Object> tradeParam) throws Exception {
		...
		try {
			...
			paramMap.put("order_amt", Convert.convert(Integer.class, tradeParam.get("orderAmt")));
	...
	private Map<String, Object> verifyCommunication(Map<String, Object> tradeParam) throws Exception {
		...
		try {
			...
			if ("0102".equals(payType)) {
				...
				resp = aliPayService.f2fScanQrCodePay(tradeParam);
				subresp = JsonUtils.JsonToMapObj((String) resp.get("conResult"));
				GlobalConfig.log_api.info("OrderCreateService 0102 收到返回：{}", resp);
				if (!"200".equals(resp.get("statuscode")) || !"0000".equals(subresp.get("returnCode"))) {
					resultMap.put("returnCode", "0008");
				} else {
					Map<String, Object> data = Convert.toMap(String.class, Object.class, subresp.get("data"));// 支付宝返回data节点
					...
					outdata.put("qrUrl", data.get("QrCodeData"));// 二维码数据
					resultMap.put("data", outdata);
				}
			} else if ("0202".equals(payType)) {
				resp = wechatPayService.f2fScanQrCodePay(tradeParam);
				subresp = JsonUtils.JsonToMapObj((String) resp.get("conResult"));
				GlobalConfig.log_api.info("OrderCreateService 0202 收到返回：{}", resp);
				if (!"200".equals(resp.get("statuscode")) || !"000000".equals(subresp.get("returnCode"))) {
					resultMap.put("returnCode", "0008");
				} else {
					Map<String, Object> data = Convert.toMap(String.class, Object.class, subresp.get("data"));// 微信返回data节点
					if (tradeParam.containsKey("secOrderId"))
						outdata.put("secOrderId", tradeParam.get("secOrderId"));
					outdata.put("orderId", tradeParam.get("orderId"));
					outdata.put("qrUrl", data.get("code_url"));// 二维码数据
					resultMap.put("data", outdata);
				}
			} else if ("0103".equals(payType) || "0104".equals(payType) || "0105".equals(payType) || "0205".equals(payType)
					|| "0203".equals(payType) || "0204".equals(payType) || "0401".equals(payType) || "06".equals(thirdId)) {
				if (tradeParam.containsKey("secOrderId"))
					outdata.put("secOrderId", tradeParam.get("secOrderId"));
				outdata.put("orderId", tradeParam.get("orderId"));
				resultMap.put("data", outdata);
